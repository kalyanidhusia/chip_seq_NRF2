...
# 
#Author   Sudin Bhattacharya 4/25/18
#Edited by  Kalyani Dhusia 2/22/2018
...
for (i in 1:length(peak_seqs)) {
  ...
  siteSet <- searchSeq(Nrf2_pfm_mod, mySeqDNAString, 
                       seqname = "seq1", min.score = "40%", strand = "*")
...
write.csv(longSites2, file = "NRF2peak_AHREs_19bp_withPeakNum.csv", row.names = FALSE)
...
system("bigWigAverageOverBed ENCFF973LBO.bigwig NRF2.bed NRF2peaks_FDR1_readSums.tab")
...
ggp10 <- ggplot(data = singletonAHREs, aes(x = peak_assoc_with, y = peak_sum)) + 
  geom_point(colour = "darkblue", alpha = 1/3) + scale_y_log10() +
  labs(x = "NRF2 peaks", y = "Area under NRF2 binding site") + 
  theme(plot.title = element_text(face = "bold", hjust = 0.5, vjust = 0, size = 14)) +
  theme(axis.title = element_text(size = 12), axis.text=element_text(size = 12))
ggsave("ggp10.png")
